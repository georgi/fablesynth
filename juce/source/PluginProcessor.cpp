...
fable::FableAgent& FableAudioProcessor::getAgent() {
    if (!agent_) {
        ...
        for (const auto& preset : factoryPresets()) presetNames.add(juce::String(preset.name));
        agent_ = fable::makeApvtsAgent(*this, apvts, info.data(), info.size(), agentStateGeneration_, [this, presetNames](codeact::Snapshot& snapshot) {
            ...
            snapshot.references = fable::agentPresetReferences("WT-1", presetNames, getCurrentProgram());
        });
    }
    return *agent_;
}
