...
void DrumHeader::paint(juce::Graphics& g) {
    ...
    // Brand sections advance by their measured tracked width. The embedded
    // Michroma face is wider than the previous fixed slots, which let DR-1
    // collide with the final H in SYNTH on the standalone's Retina surface.
    ...
    drawSpaced(g, "DR-1", brand.withTrimmedTop(4), 2.0f);
