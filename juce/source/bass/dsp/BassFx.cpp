...
void BassFx::prepare(double sampleRate) {
    ...
    // node, so match it here or the plugin sits several dB under the web app.
...
void BassFx::process(float* L, float* R, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // ---- drive (2x oversampled tanh waveshaper, post-accent) ----
        if (!driveGated_) {
        ...
        // ---- safety limiter (feed-forward peak compressor) ----
        {
            ...
            // static curve: threshold -6 dB, ratio 14 (hard knee; the web's
