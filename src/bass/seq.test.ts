...
import {
  ...
  slidesInto, STEP_STRIDE, stepDurSamples, STEPS, stepSemi, swingDelaySamples,
...
describe('bass seq model', () => {
  it('empty patterns read back as neutral rests', () => {
    ...
    expect(s).toEqual({ on: false, note: 0, oct: 0, acc: false, slide: false });
  });

  it('setStep/getStep round-trips all fields', () => {
    let p = makeEmptyPatterns();
    p = setStep(p, 1, 3, { on: true, note: 10, oct: -1, acc: true, slide: true });
    expect(getStep(p, 1, 3)).toEqual({ on: true, note: 10, oct: -1, acc: true, slide: true });
    ...
    expect(getStep(p, 1, 3)).toEqual({ on: true, note: 10, oct: -1, acc: false, slide: true });
  });
  ...
  it('slidesInto requires both steps on and the target flagged', () => {
    const on = { on: true, note: 0, oct: 0, acc: false, slide: false };
    const sld = { ...on, slide: true };
    const off = { ...on, on: false };
    expect(slidesInto(on, sld)).toBe(true);
    expect(slidesInto(off, sld)).toBe(false);
    expect(slidesInto(on, on)).toBe(false);
    expect(slidesInto(on, { ...sld, on: false })).toBe(false);
  });
  ...
  it('randomPattern yields 16 valid steps and never slides step 0', () => {
    for (let trial = 0; trial < 20; trial++) {
      const steps = randomPattern();
      expect(steps.length).toBe(STEPS);
      expect(steps[0].slide).toBeFalsy();
